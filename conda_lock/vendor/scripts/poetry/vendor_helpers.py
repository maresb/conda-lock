...
class DependencyData(BaseModel):
    ...
    def describe(self) -> str:
        ...
        if package_name == "jsonschema":
            # There are two license files, but with different years.
            # Let's just use the earliest year.
        ...
        if package_name == "packaging":
            ...
            assert all(license.is_mit for license in licenses)
            license_description = "MIT"

        description = (
            f"{package_name} v{self.version}, "
            f"licensed as {license_description}, {copyright_line}."
        )
        return description
...
def req_to_req_obj(req: str, dep: DependencyData) -> Requirement | None:
    ...
    if python_requirements is None:
        ...
        # Read the Python version requirements from pyproject.toml
        pyproject_toml = toml.loads((get_repo_root() / "pyproject.toml").read_text())

        # '>=3.6'
        conda_lock_requires_python = pyproject_toml["project"]["requires-python"]
