"""Lockfile-side heal of carry-forward mamba 2.1.1-2.5 corruption.

When a previous ``conda-lock lock`` ran against a corrupt
``repodata_record.json`` cache (mamba/micromamba 2.1.1-2.5,
...
def find_cache_index_for_locked_dep(
    ...
    at install time and is unaffected by the mamba 2.1.1-2.5
...
def _parse_match_spec_to_name_version(spec_str: str) -> tuple[str, str]:
    ...
    >>> _parse_match_spec_to_name_version("__glibc >=2.17,<3.0.a0")
    ('__glibc', '>=2.17,<3.0.a0')
    >>> _parse_match_spec_to_name_version("tzdata")
    ('tzdata', '')
    """
...
def heal_locked_dependencies_from_cache(
    ...
    A lockfile generated against a corrupt cache (mamba 2.1.1-2.5,
    ...
    - Cache confirms canonical depends are non-empty -> heal in place
      ...
      2.1.1-2.5 corruption for *this* entry.
