...
def test_is_mamba_2x_corrupt_record_signature():
    ...
    assert is_mamba_2_1_to_2_5_stub_record(
    ...
    assert not is_mamba_2_1_to_2_5_stub_record(
        {
...
def test_get_repodata_record_unhealable_corrupt_beats_rejection(tmp_path):
    """Outcome priority: if any candidate carried the mamba 2.1.1-2.5
    ...
    Layout: hierarchical candidate is a mamba 2.1.1-2.5 stub with
    ...
    """
    pkgs = tmp_path / "pkgs"
    # Hierarchical: corrupt + unhealable. The stub preserves url /
    # md5 / fn like the real bug does (sha256 is dropped by the
    # bug, but we include it here to make the flat impostor's
    # sha256 mismatch below unambiguous), so it survives the
    # identity precondition for ``unhealable_corrupt``.
    hier_info = (
        pkgs
        / "https/conda.anaconda.org/conda-forge/linux-64"
        / "libzlib-1.3.2-h25fd6f3_2"
...
def test_get_repodata_record_unhealable_requires_identity_match(tmp_path):
    ...
    # different package and looks like a 2.1.1-2.5 stub.
    flat_info = pkgs / "libzlib-1.3.2-h25fd6f3_2" / "info"
    flat_info.mkdir(parents=True)
    (flat_info / "repodata_record.json").write_text(
        json.dumps(
...
def test_get_repodata_record_unhealable_accepts_single_strong_field(tmp_path):
    ...
    mamba 2.1.1-2.5 stub carries ``url`` + ``md5`` + ``fn`` (the
    bug drops ``sha256``), so requiring all four would reject every
    real stub; requiring any one accepts them all while still
    demanding positive artifact identity.
    """
    pkgs = tmp_path / "pkgs"
    flat_info = pkgs / "libzlib-1.3.2-h25fd6f3_2" / "info"
    flat_info.mkdir(parents=True)
...
def test_heal_corrupt_record_returns_none_on_unreadable_index_json(tmp_path):
    """An unparseable ``info/index.json`` means no heal evidence: the
    record stays unhealed (None) instead of raising."""
    record_dir = tmp_path / "pkg" / "info"
    record_dir.mkdir(parents=True)
    record_path = record_dir / "repodata_record.json"
    record_path.write_text("{}")
    (record_dir / "index.json").write_text('{"broken": ')
    assert heal_corrupt_record({"timestamp": 0, "license": ""}, record_path) is None
