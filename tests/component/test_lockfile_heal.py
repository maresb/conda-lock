...
against a corrupt mamba 2.1.1-2.5 cache, the entries it carries
...
from conda_lock.solver.lockfile_heal import (
    LockfileHealReport,
    _index_matches_link,
    find_cache_index_for_locked_dep,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Unknown extension: kept verbatim rather than guessed at.
        ("libzlib-1.3.2-h25fd6f3_2", "libzlib-1.3.2-h25fd6f3_2"),
    ],
)
def test_locked_dep_as_link_action_handles_other_extensions(
    fn: str, expected_dist_name: str
):
    """dist_name derivation strips ``.tar.bz2`` like ``.conda`` and
    passes unknown filenames through unchanged."""
    dep = _corrupt_lockfile_entry(fn=fn)
    _link_action, dist_name = locked_dep_as_link_action(dep)
    assert dist_name == expected_dist_name


def test_index_matches_link_rejects_name_and_version_mismatch():
    """Positive name/version contradictions each reject the index."""
    link = {"name": "libzlib", "version": "1.3.2"}
    assert _index_matches_link({"name": "zlib", "version": "1.3.2"}, link) is False
    assert _index_matches_link({"name": "libzlib", "version": "1.3.3"}, link) is False
    assert _index_matches_link({"name": "libzlib", "version": "1.3.2"}, link) is True


def test_find_cache_index_skips_unparseable_index_json(tmp_path: Path):
    """A garbled ``info/index.json`` yields no evidence (ambiguous),
    not an exception."""
    dep = _corrupt_lockfile_entry()
    link_action, dist_name = locked_dep_as_link_action(dep)
    pkgs_dir = tmp_path / "pkgs"
    info_dir = pkgs_dir / dist_name / "info"
    info_dir.mkdir(parents=True)
    (info_dir / "index.json").write_text('{"cut off": ')
    assert find_cache_index_for_locked_dep([pkgs_dir], dist_name, link_action) is None
